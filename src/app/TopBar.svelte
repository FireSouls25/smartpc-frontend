<script lang="ts">
  ...
  const themeLabel = (v: Theme): string =>
    v === "auto" ? t("common.auto") : v === "light" ? t("common.light") : t("common.dark");
</script>
...
<header class="flex shrink-0 items-center justify-between gap-3 py-4">
  ...
  <div class="flex items-center gap-2">
    <div class="relative" data-menu>
      <button class="icon-btn" onclick={() => toggle("theme")} aria-label={t("common.theme")}>
        {#if getTheme() === "dark"}
        ...
        <div class="dropdown" role="menu">
          {#each themes as v (v)}
            ...
            >
              <span
                class="dot"
                style="background: {getTheme() === v ? 'var(--accent)' : 'var(--border-strong)'};"
              ></span>
              {themeLabel(v)}
            </button>
          {/each}
    ...
    <div class="relative" data-menu>
      <button class="icon-btn" onclick={() => toggle("lang")} aria-label="Language">
        <svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="dropdown" role="menu">
          {#each langs as v (v)}
            ...
            >
              <span
                class="dot"
                style="background: {getLang() === v ? 'var(--accent)' : 'var(--border-strong)'};"
              ></span>
    ...
    <button class="icon-btn" onclick={onSettings} aria-label={t("topbar.settings")}>
      <svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
